...
ydata=cell(Tsize,1); 
Ydata=cell(Tsize,1); Ydata_X=zeros(Defsize,1); Ydata_Y=zeros(Defsize,1);
ydatacrpd=cell(Tsize,1); ydatacrpdR1=cell(Tsize,1); 
Ampguess=zeros(Tsize,1); Case=cell(Defsize,1); 
x0=cell(Defsize,1); x=cell(Defsize,1); X=cell(Defsize,1); 
Y=cell(Defsize,1); Yg=zeros(Tsize,Defsize); Xg=zeros(Tsize,Defsize); 
Grid=cell(2,Defsize);
...
% parameters : [Amplitude, x0, sigmax, y0, sigmay]
...
%     ydatacrpd{i}=ydata{i};
   ...
   %still needs background level to be mediated from within the channel.   
    ...
    [x{j}(i,:),resnorm,residual,exitflag] = lsqcurvefit(@GaussPlosFunc, ...
       x0{j}(i,:),Grid(:,j),Ydata{i,j},lb,ub,OPTIONS);
...
II=cell(5,5,Tsize);
FCII=cell(1,Tsize);
ROI=cell(Tsize,Defsize);
Ispot=zeros(Tsize,Defsize);

%SNR vars
SNR=zeros(Tsize,Defsize);
SNRSimonetti=zeros(Tsize,Defsize);
SNRBlur=zeros(Tsize,Defsize); %theoretical SNR of noise simulator
