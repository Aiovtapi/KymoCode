...
%point. 
    ...
    actions.saveresults=0;
    
    initval.difstep=8;          %diffusion step
    initval.c0=30;              %columns picture
    initval.r0=30;              %rows,in pixels
    initval.blowup=11;          %To suppress sampling bias
    initval.spotno=1;          %number of spots
    initval.psf=1.3;            %point spread function
  ...
  if actions.saveresults      
        mpth='D:\jkerssemakers\My Documents\BN ND Recent\BN_ND11_CharlBacterialReplication\2015_02 FociEval\ImageDatabase\';
...
if actions.plotintermediates
  ...
  if actions.saveresults
        nm1=strcat(nwpth,'blobsimage',num2str(jj,'%02.0f'), '.tif');
        imout=uint16(camera_pic_blobs-1);
