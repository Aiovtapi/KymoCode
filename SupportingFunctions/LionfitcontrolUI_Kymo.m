function varargout = LionfitcontrolUI_Kymo(varargin)
...
handles.initval = varargin{1};
handles.Bacpics = varargin{2};
handles.cells = varargin{3};
handles.chan = varargin{4};

handles.IPTP = 1.5;
finddif = strcmp(handles.initval.difchan,handles.initval.viewchan);
viewFLchans = {handles.initval.viewchan{finddif},handles.initval.viewchan{~finddif}};

Xout = GaussFitSimedit_KymoControl(handles.Bacpics,1,handles.IPTP);

title = strcat('Tweak Intensity Peak Threshold for ',viewFLchans{handles.chan});
...
Xout = GaussFitSimedit_KymoControl(handles.Bacpics,C1,handles.IPTP);
