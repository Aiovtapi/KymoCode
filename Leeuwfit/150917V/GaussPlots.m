...
txtbox7={TotCellsStr};
xcc=linspace(0,1,MaxBacLife);
xccd=linspace(0,1,MaxBacLifed);
...
figure(8)
hold on
plot(xcc,M(:,7),'b','LineWidth',4)
plot(xcc,M(:,1)+MR1(:,1)+MR2(:,1)+MR3(:,1),'r','LineWidth',4)
plot(xcc,M(:,7)+M(:,8),'--b')
plot(xcc,M(:,7)-M(:,8),'--b')
plot(xcc,M(:,1)+MR1(:,1)+MR2(:,1)+MR3(:,1)+sqrt(M(:,6).^2+MR1(:,6).^2+MR2(:,6).^2+MR3(:,6).^2),'--r')
plot(xcc,M(:,1)+MR1(:,1)+MR2(:,1)+MR3(:,1)-sqrt(M(:,6).^2+MR1(:,6).^2+MR2(:,6).^2+MR3(:,6).^2),'--r')
hold off
